...
void Http::set_log(void(*log)(const char *, ...)) {
    this->log = log;
};

#ifdef HAVE_SSL
void Http::set_use_ssl(bool use) {
    this->useSSL = use;
};
#endif

int Http::connect(const char *host, int port) {
    int ret;
    Address addr;
...
int Http::connect(const TcpSockAddr& sock) {
    int ret;
...
int Http::header(const char *attrName, const char *attrValue) {
    return request.set_attr(attrName, attrValue);
};

int Http::auth(const char *user, const char *password) {
    const char *base64;

    snprintf(buf, sizeof(buf), "%s:%s", user, password);
    base64 = base64_encode(buf);
    snprintf(buf, sizeof(buf), "Basic %s", base64);
    delete[] base64;

    return request.set_attr("Authorization", buf);
};

int Http::proxy_auth(const char *user, const char *password) {
    const char *base64;
...
int Http::set_range(off_t start, off_t end) {
    if (end < 0) {
        snprintf(buf, sizeof(buf), "bytes=%lld-", start);
    } else {
...
const char* Http::get_header(const char *attr_name) {
    return response.get_attr(attr_name);
};

off_t Http::get_file_size() {
    return fileSize;
};
...
int Http::read_data(char *buffer, int maxsize) {
    int ret;
...
int Http::data_ends() {  // not very reasonable
