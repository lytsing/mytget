...
int Ftp::connect(const char *addrstr, int port) {
    assert(addrstr);
...
int Ftp::connect(const TcpSockAddr& sock) {
    int ret;
...
int Ftp::ftp_cmd(const char* cmd, const char* args) {
    int ret;
    char buffer[1024];
    int len;
...
int Ftp::rest(off_t offset) {
    ...
    snprintf(buffer, sizeof(buffer), "%lld", offset);
...
int Ftp::set_mode(int mode) {
    if (mode != PASV && mode != PORT) return -1;
    this->mode = mode;
    return 0;
};

void Ftp::set_log(void(*f)(const char*str, ...)) {
    this->log = f;
};

void Ftp::set_timeout(long timeout) {
...
int Ftp::port(int port) {
    ...
    if (localAddr.ai_family == AF_INET) {
        ...
        if (localAddr.get_addr(addr, INET6_ADDRSTRLEN) < 0)
            return -1;
        snprintf(buffer, sizeof(buffer), "|2|%s|%d|", addr, port);
...
// opened successfully
int Ftp::ftp_data_cmd(const char* cmd, const char* args, off_t offset) {
    int ret;
    int port;
...
int Ftp::read_data(char *buffer, int maxsize) {
    return dataConn->read(buffer, maxsize, timeout);
};

// get a line, will be used by LIST
int Ftp::gets_data(char *buffer, int maxsize) {
...
int Ftp::list(const char *dir) {
    return ftp_data_cmd("LIST", dir);
};

int Ftp::retr(const char *file, off_t offset) {
    if (!file) return -1;
