 ...
 * *******************************/
int Address::resolve(const char *dns_name, int port, int family) {
    char serv[12];
    int n;
    ...
    if (snprintf(serv, sizeof(serv), "%d", port) < 0) return -1;
...
int timeout_connect(int sock_fd, struct sockaddr* serv_addr, int addrlen, long tsec) {
    int flag, error, ret;
