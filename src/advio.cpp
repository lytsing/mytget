...
IOStream::IOStream(int infd) :fd(infd) {
#ifdef HAVE_SSL
    ssl = NULL;
    sslCTX = NULL;
    useSSL = false;
...
int IOStream::open(const char *file, int flag, mode_t mode) {
...
#ifdef HAVE_SSL
void IOStream::set_use_ssl(bool use) {
    if (use) {
        if (!useSSL) {
            useSSL = true;
            SSL_free(ssl);
 ...
 ****************************************************************/
BufferStream::BufferStream(int ifd) : IOStream(ifd) {
    ptr = buf;
    count = 0;
};

BufferStream::BufferStream(const BufferStream& that) : IOStream(-1) {
    ptr = buf;
    count = 0;
};

BufferStream& BufferStream::operator = (const BufferStream& that) {
...
int BufferStream::set_fd(int infd) {
    buf[0] = '\0';
    ptr = buf;
    count = 0;
...
int BufferStream::write(char *str, long timeout) {
    int n, rc, slen;
    char *pptr;
...
int BufferStream::read(char *buffer, int maxsize, long timeout) {
    if (count > 0) {
        int min;
...
int PluginIO::read_data(char *buffer, int maxsize) {
...
int BufferFile::open(const char *file, int flag, mode_t mode) {
    ptr = buf;
    left = FILE_BUFFER_SIZE;
    return ( fd = ::open(file, flag, mode) );
};
...
off_t BufferFile::seek(off_t off_set) {
...
int BufferFile::truncate(off_t length) {
...
off_t BufferFile::retr_data_from(PluginIO *pio, off_t *rtlength, off_t length) {
    assert(pio != NULL);
    int ret;
    off_t data_count = 0;
